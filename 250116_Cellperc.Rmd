...
INPUT_PATH = file.path(BASE, "Data/Cellperct_input")
...
celldata = read.csv(paste0(INPUT_PATH, "/250221_celldata.csv"))
...
levels_annotation = c("T cell CD4", "T cell CD8", "Tregs","B cells", "Neutrophils", "Macrophages", "Monocytes", "Dendritic cells","Dendritic cells CD103", "Endothelium","Epithelium", "Fibroblasts","Tumour","Uncertain")
...
plot.celltypeperc(celldata = celldata[!celldata$annotation %in% c("Tumour",  "Fibroblasts", "Endothelium","Uncertain","Epithelium"),],
...
## consider that cluster 22 are mainly distributed in 08WT maybe redo the phenocluster for macrophages 

```{r}
## try k200
clustCSV_mac = read.csv(file.path(INPUT_PATH, "250221_clustering_k200_k100_k50_9markers_mac.csv"))
celldata_macrophages = cbind(celldata_macrophages, clustCSV_mac)
plot.celltypeperc3 = function(celldata, title) {
  date = str_sub(gsub("-", "", Sys.Date()), -6, -1)

p = ggplot(celldata,
             aes(x = factor(ROI_ID, levels = unique(ROI_ID)),
                 fill = factor(k200, levels = unique(k200)))) +
    geom_bar(position = "fill", colour = "black") +
    scale_y_continuous(labels = scales::percent) +
     scale_fill_manual(values = random_colours) +
    theme_classic() +
...
plot.celltypeperc3(celldata = celldata_macrophages,title = "Stacked_macrophages_ROI_K200")
plot.celltypeperc3(celldata = celldata_macrophages,title = "Stacked_macrophages_ROI_K200")

# k300
clustCSV_mac = read.csv(file.path(INPUT_PATH, "250221_clustering_k400_k300_9markers_mac.csv"))
celldata_macrophages = cbind(celldata_macrophages, clustCSV_mac)
plot.celltypeperc3 = function(celldata, title) {
  date = str_sub(gsub("-", "", Sys.Date()), -6, -1)
...
p = ggplot(celldata,
             aes(x = factor(ROI_ID, levels = unique(ROI_ID)),
                 fill = factor(k300, levels = unique(k300)))) +
    geom_bar(position = "fill", colour = "black") +
    scale_y_continuous(labels = scales::percent) +
     scale_fill_manual(values = random_colours) +
    theme_classic() +
...
plot.celltypeperc3(celldata = celldata_macrophages,title = "Stacked_macrophages_ROI_K300")
plot.celltypeperc3(celldata = celldata_macrophages,title = "Stacked_macrophages_ROI_K300")
...
ggplot(celldata, aes(x= Location_Center_X, y = -Location_Center_Y)) +
  ...
  geom_point(data = celldata[which(celldata$MI_CXCL9 >= 0.5),], size = 0.01, colour = "yellow") +
  theme_classic() +
...
ROI13<-subset(celldata, ROI_ID == "09_WT") #   [1] "01_CCR2KO" "02_CCR2KO" "03_CCR2KO" "04_CCR2KO" "05_CCR2KO" "06_WT"     "07_WT"     "08_WT"     "09_WT"    "10_WT"     "11_WT"  
ggplot(ROI13, aes(x= Location_Center_X, y = -Location_Center_Y)) +
  ...
  geom_point(data = ROI13[which(ROI13$MI_CXCL9 >= 0.5),], size = 1, colour = "red")+
...
stat_250226_ROI = celldata %>%
 # group_by(ExpGroup,MouseID,ROI_ID) %>%
  group_by(ExpGroup,MouseID,ROI_ID) %>%
    ...
    DCothern = sum(annotation =="Dendritic cells"),
    DCn =sum(annotation %in% c("Dendritic cells CD103","Dendritic cells")),
    ...
    CD86DCn = sum(CD86exp=="CD86+" & annotation %in% c("Dendritic cells CD103","Dendritic cells")),
    ...
    MHCIIDCn = sum(MHCIIexp=="MHCII+" & annotation %in% c("Dendritic cells CD103","Dendritic cells")),
    ...
    PDL1DCn = sum(PDL1exp=="PDL1+" & annotation %in% c("Dendritic cells CD103","Dendritic cells")),
...
plot.boxplot.ROI(celldata = stat_250226_ROI, comparisons = my_comparisons)
```
...
levels_annotation = c("T cell CD4", "T cell CD8", "Tregs", "B cells","Neutrophils", "Macrophages","Monocytes", "Dendritic cells CD103",  "Dendritic cells", "Endothelium", "Epithelium", "Fibroblasts","Tumour")
...
plot.celltypeperc <- function(celldata, title) {
  ...
  p <- ggplot(
    ...
    scale_fill_manual(
      ...
      values =  c("B cells" = "#945931",  "Dendritic cells" = "#FF9933", "Dendritic cells CD103" = "#FFCC66", "Endothelium" = "#FFEA42",
              "Epithelium" = "#FFF4A4FF", "Fibroblasts" = "#ABDDA4FF",  "TCRgdt" = "#336666","Macrophages" = "#4E79A7FF",
              "Macrophages F480" = "#4E79A7FF", "Macrophages CD68" = "#5F9EA0", "Macrophages CD11b" = "#0072B2", "Neutrophils" = "#A0CBE8FF",  "T cell CD4" = "#B07AA1FF","Monocytes"= "#40E0D0",  "T cell CD8" = "#FF9D9AFF", "Tregs" = "#CC6666", "Tumour" = "#704850FF", "Uncertain" = "#8C8C8C")
...
# Call the function
plot.celltypeperc(celldata = celldata_CCR2, title = "Stacked_domain_CCR2_Domain")
plot.celltypeperc(celldata = celldata_WT, title = "Stacked_domain_WT_Domain")
# plot.celltypeperc(celldata = celldata[!celldata$annotation %in% c("Tumour", "Uncertain"),],
#                   title = "Stacked_without_tumour")
...
## Tumour domain
stat_250226_Tumourdomain = celldata_Tumour %>%
 # group_by(ExpGroup,MouseID,ROI_ID) %>%
   group_by(ExpGroup,MouseID,ROI_ID) %>%
    dplyr::summarise(
    totaln = n(),
 ...
 # group_by(MouseID, ROI_ID, ExpGroup) %>%
  ...
  ungroup()
## interface doamin 
stat_250226_Interfacedomain = celldata_Interface %>%
 # group_by(ExpGroup,MouseID,ROI_ID) %>%
   group_by(ExpGroup,MouseID,ROI_ID) %>%
    dplyr::summarise(
    totaln = n(),
 ...
 # group_by(MouseID, ROI_ID, ExpGroup) %>%
  ...
  ungroup()
## Normal domain
stat_250226_Normaldomain = celldata_Normal %>%
 # group_by(ExpGroup,MouseID,ROI_ID) %>%
   group_by(ExpGroup,MouseID,ROI_ID) %>%
    dplyr::summarise(
    totaln = n(),
...
plot.boxplot.domain = function(celldata, comparisons = list(c("WT", "CCR2KO"))) {
  numeric_cols <- celldata %>% select(where(is.numeric)) %>% colnames()
  for (factor in numeric_cols) {
    factor_sym <- ensym(factor)  
    is_percentage <- max(celldata[[factor]], na.rm = TRUE) <= 1.5
    ...
    date <- Sys.Date()
    ggsave(filename = paste0(date, "_Boxplot_", factor, ".jpeg"),
           path = file.path(OUTPUT_PATH, "Boxplots_percentage/Domain/Interfacedomain"),
           plot = p, bg = "white",
           width = 2100, height = 1700, units = "px")
  }
}

plot.boxplot.domain(celldata = stat_250226_Normaldomain, comparisons = my_comparisons)
plot.boxplot.domain(celldata = stat_250226_Tumourdomain, comparisons = my_comparisons)
plot.boxplot.domain(celldata = stat_250226_Interfacedomain, comparisons = my_comparisons)
...
date <- Sys.Date()
output_file = paste0(OUTPUT_PATH, "stat_250226_Interfacedomain.csv")
output_file
write.csv(stat_250226_Interfacedomain, file = output_file, row.names = F) 

output_file = paste0(OUTPUT_PATH, "stat_250226_Tumourdomain.csv")
write.csv(stat_250226_Interfacedomain, file = output_file, row.names = F) 

output_file = paste0(OUTPUT_PATH, "stat_250226_Normaldomain.csv")
write.csv(stat_250226_Interfacedomain, file = output_file, row.names = F) 
