...
##############################
## check the inpt data 
celldistance_calculation<-read.csv("/mnt/Data1/groupfebe/runs/Xiaofei/CCR2_LUNG/Results/Spatial_output/20250526_allROIs_25px_CCR2LUNG_neighbours_distanceCalculation_allCells.csv")
if (length(unique(celldistance_calculation$ROI_ID)) == 12) {
  print("可以计算frequence")
} else {
  print("ROI数量不足12个，建议检查数据")
}
dim(celldistance_calculation)

# Name of data file to read in 
path = "/mnt/Data1/groupfebe/runs/Xiaofei/CCR2_LUNG"

filename = "frequencies_25px_20250523.csv"
...
selectcolumns = c(grep("B cells", names(celldata_fre), ignore.case = TRUE),
                  ...
                  grep("Monocytes", names(celldata_fre), ignore.case = TRUE),
                  ...
                  grep("Uncertain", names(celldata_fre), ignore.case = TRUE), 
...
write.csv(celldata_fre, paste(path, output_path, date,"Rphenograph_CCR2LUNG_output_", max(celldata_fre$cluster), "clusters_k300_", columns_of_interest, "ct_fractions.csv", sep = ""))
print("file saved")
